...
#     ]
#     return df_results[["var", "clean_print"]]
...
#     X_train, y_train, X_test, y_test = tt_split(train_df, val_df, covariates, pred_var)
#     if not model_to_load:
#         cluster = train_df[cluster_var] if cluster_var is not None else None
#         model = fit_sm_regression(X_train=X_train, y_train=y_train, logreg=logreg, cluster=cluster)
#         if model_save_path:
...
#         big_model = model
#         X_train_s, y_train_s, _, _ = tt_split(
#             train_df, val_df, list(set(covariates).difference(set(f_covars))), pred_var
#         )
#         small_model = fit_sm_regression(X_train=X_train_s, y_train=y_train_s, logreg=logreg)
#         f_stat = calculate_nested_f_statistic(small_model, big_model)
#     # summarize (the reason this function exists)
#     if print_standard:
#         print(model.summary())
#         return y_test, y_pred_proba
#     if print_vars:
...
#                     ["f_stat", f_stat],
#                     ["ROC", round(roc_auc_score(y_test, y_pred_proba), 4)],
#                     #["Precision", round(precision_recall_fscore_support(y_test, highrisk)[0], 4)],
#                     #["Recall", round(precision_recall_fscore_support(y_test, highrisk)[1], 4)]
#                     #["F-Score", round(precision_recall_fscore_support(y_test, highrisk)[2], 4)]
#                     ["R2", round(model.pseudo_rsquared(), 4)],
#                 ],
#                 columns=coefs.columns,
#             )
...
#             plt.savefig(f'predictions/regression_model/ROC_AUC_scd1hat')
#             #plt.show()

#             #plot calibration curve
#             y, x = calibration_curve(y_test, y_pred_proba, n_bins=50)
#             # plot perfectly calibrated
#             plt.figure()
#             plt.plot([0, 0.5], [0, 0.5], linestyle='--')
#             # plot model reliability
#             plt.plot(x[:25], y[:25], marker='.')
#             plt.xlabel('Mean predicted value')
#             plt.ylabel('Fraction of positives')
#             plt.title('Calibration curve')
#             plt.savefig(f'predictions/regression_model/Calibration_Curve_scd1hat')
...
def sm_regress_mechanism(
    ...
    # Extract variables for SCD mechanism table
    df_results = pd.read_html(
        model.summary(yname=pred_var, xname=["intercept"] + covariates).tables[1].as_html(), header=0, index_col=0
    )[0].reset_index()
    df_results.columns = ["var", "coef", "se", "z", "p", "25", "75"]
    results_of_interest = df_results.loc[df_results['var']=="scd1_hat"]
    coef, se_val, p = results_of_interest['coef'], results_of_interest['se'], results_of_interest['p']
    AUC_val = roc_auc_score(y_test, y_pred_proba)

    return coef, se_val, p, AUC_val